...
def test_search_algorithm(graph_name, start_node, end_node, expected_path, expected_expand_count):
    expand.expand_count = 0
    graph = globals()[graph_name]
    path = sc.breadth_first_search(graph, start_node, end_node)
    assert path == expected_path
    assert expand.expand_count == expected_expand_count
